function mutation_frequency_plot( input , name )
...
map=vivid(M*(N_circle+1),basiccolor(1:M),[0.5 1]); % light color based on rgbm
colormap(map);
%colormap(gray)
...
for m = 1 : M
    ...
    [C,h(m)] = contour(x,y,PMATRIX,'LineWidth',2);
    hold on
    
    set( h(m) , 'ShowText','off' ,...
        'LevelList',cut_density_p(1:N_circle))
...
%            set(Cld(j),'cdata',((m-1)*(N_circle+1)+j)*ones(size(get(Cld(j),'cdata')))); 
            set(Cld(j),'EdgeColor',map((m-1)*(N_circle+1)+j,:));
            if j > 1
                set(Cld(j),'LineStyle','--','LineWidth',1);
            end
        end
    end
...
for m = 1 : M
    ...
    x2=input(m,3);
    N2=input(m,4);
    plot( x2/N2 , x1/N1 , 'x' , 'color' , map((m-1)*(N_circle+1)+1,:) ,'linewidth',3)
    %   text( x2/N2 , x1/N1 , name{m} ,'fontsize',16 )
    text(x2/N2 , x1/N1,[' <--------------- ',name{m}],'FontName','Arial','FontSize',18,'color',map((m-1)*(N_circle+1)+1,:))
end